<template>
    <div class="apply_order">
            ...
            <div>
                <div class="upload">
                    ...
                    <el-dialog v-model="state.dialogVisible">
                        ...
                        " w-full style="width: 100%; height: 100%" controls autoplay :src="state.dialogImageFile.url">
        ...
        <div v-if="userInfo.isSuper === 1">
            <!-- A13店长复核中&A03审核拒绝时不能再点击按钮 -->
            <el-button type="primary" v-if="applyInfoData.applyStatus ==='A13' && applyInfoData.guestStatus !=='A03'" @click="storeManagerReview('pass')">
                店长复核通过
            </el-button>
            <el-button type="primary" v-if="applyInfoData.applyStatus ==='A13' && applyInfoData.guestStatus !=='A03'" @click="downAccountdialogRef.show();">
                店长复核拒绝
            </el-button>
        </div>
        <!-- 拒绝 弹框 -->
        <common-dialog ref="downAccountdialogRef" :headerTitle="''" @confirm="storeManagerReview('reject')"
...
import { downAccountPass, downAccountReject, storeManagerReviewPass, storeManagerReviewReject } from "@/api/downAccount";
...
// 店长复核
const storeManagerReview = async (status: any) => {
    // 拒绝
    if (status === 'reject') {
...
const updatedownAccountStatus = async (status: any) => {
    ...
    if (status === 'pass') {
        await storeManagerReviewPass(params).then((res: any) => {
            if (res.code === 200) {
                ElMessage({
                    message: "店长复核通过",
                    type: 'success',
        ...
        await storeManagerReviewReject(params).then((res: any) => {
            if (res.code === 200) {
                ElMessage({
                    message: "店长复核通过",
                    type: 'success',

...
import { cmsDetail, cmsList, cmscategoryList, configList } from "@/api/officialSite";
const columns = ref([
    { width: '120', key: "k", label: 'key' },
    { width: '120', key: "v", label: '值' },
    { width: '120', key: "type", label: '类型' },
    { width: '120', key: "remark", label: '备注' },
    // { width: '120', key: "status", label: '状态' },
]);
...
const init = async () => {
    ...
    await configList(params).then((data) => {
        console.log(data);
        if (data.code === 200) {
            categoryListData.value = data.data;
